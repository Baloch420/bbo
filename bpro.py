#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Mubashar Baloch
# Update V0.1

# Copyright© Baloch ID 2021
# 100% Open Source Code

# Author : Mubashir Baloch
# Facebook (Zait Ullah)   : Facebook.com/MUB4SH4R
# Warning (☬ Mr Baloch ☬)    : Tricker Baloch Here Baby
# Whatsapp (Mr Baloch )      : +923470336477
# Warning (MUB4SH4R)           : Hatrz Feel Me
...
_logo_line_2_ = ('%s| ___ \/ _ \ | |   |  _  /  __ \| | | | %s┌─────────────────────────┐'%(_P_,_U_))
...
### Tampilan Metode
def start_method():
    _dapunta_cici_('\n%s[%s1%s] %sBaloch Api'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sBaloch Mbasic'%(_U_,_P_,_U_,_P_))

### Tampilan Mulai Crack
def started():
    _dapunta_cici_('%s[%s•%s] %sBaloch Start Cracking...'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s•%s] %sBaloch [OK]  OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sBaloch [CP]  CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
